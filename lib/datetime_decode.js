...
exports.getTimeStamp = function(dateStrings, secondsFromEpoch, fraction) {
  const milliseconds = fraction?.milliseconds || 0;
  const dateObject = new Date(secondsFromEpoch * 1000 + milliseconds);

  // Verifica se a opção de string para timestamp está habilitada
  if (!useDateStringsForType(dateStrings, 'TIMESTAMP')) {
    return dateObject;
  }

  // Retorna a data padrão do MySQL quando o timestamp é zero
  if (secondsFromEpoch === 0 && (!fraction || fraction.value === 0)) {
    return `0000-00-00 00:00:00${getFractionString(fraction)}`;
  }

  // Retorna a string formatada com o timestamp
  return `${getDateString(dateObject.getFullYear(), dateObject.getMonth() + 1, dateObject.getDate())} ` +
