  ...
  switch (column.type) {
    ...
    case MysqlTypes.VARCHAR:
      case MysqlTypes.STRING:
        ...
        const defPrefix = def.slice(0, 6);
        
        if (defPrefix === 'binary') {
          result = Buffer.alloc(parseInt(def.slice(7, def.length - 2), 10));
          parser.parseBuffer(size).copy(result);
        } else if (defPrefix === 'varbin') {
          result = parser.parseBuffer(size);
        } else {
          if (['utf8mb3', 'utf8mb4', 'utf16'].includes(column.charset)) {
    ...
    case MysqlTypes.BLOB:
      ...
      if (column.charset !== null) {
        ...
        if(column.charset === 'utf8mb3' || column.charset == 'utf8mb4' || column.charset == 'utf16')
          column.charset = 'utf8'
        else
